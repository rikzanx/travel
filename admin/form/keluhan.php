...
<td> <input type = "text" name="id_user" id="id_user" maxlength="8" size="8" value="<?php  echo trim ($row["id_user"])?>"></td>
</tr>
</table>
<?php  }?>
...
$mulai_dari = $limit * ($page -1);
  $sql = " select * from keluhan, user where keluhan.id_user=user.id_user limit $mulai_dari,$limit " ;
...
function sql_insert()
{
  ...
  echo $_POST["d_keluhan"];
   ...
   if($_POST["j_keluhan"]== "" || empty($_POST["j_keluhan"])){ 
    ...
    }elseif  ($_POST["id_user"]== "" || empty($_POST["id_user"])){
      echo " <div class='row'>
                    <div class='col-lg-12'>
                        <div class='alert alert-danger alert-dismissable'>
                            <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
  ...
  $sql  = " insert into `keluhan` (`id_user`,`j_keluhan`,`d_keluhan`) 
...
function sql_update()
{
  global $kdb1;
  global $_POST; 
 $sql  = " update  `keluhan` set `id_user` = '".$_POST["id_user"]."',`j_keluhan` = '".$_POST["j_keluhan"]."',
