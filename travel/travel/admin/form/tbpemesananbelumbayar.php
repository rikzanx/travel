...
$id_user = !empty($_GET['user']) ? $_GET['user'] : " ";   
//$kdb = koneksidatabase();
$a = @$_GET["a"];
$sql = @$_POST["sql"];
switch ($sql) {
...
<a href="index.php?form=13&a=detail&id=<?php echo $baris['id_pesan']; ?>&user=<?php echo $baris['id_user']; ?>"class="btn btn-default">DETAIL</a>   
  </td>
  </tr>
  <?php
   $i++;  
...
<?php  }?>
  ...
  <?php } ?>
        ...
        $sql = " select * from pesan, jadwal, stasiun_asal, stasiun_tujuan, user where 
          pesan.id_jadwal = jadwal.id_jadwal and pesan.id_user = user.id_user and jadwal.id_stasiun_asal = stasiun_asal.id_stasiun_asal
          and jadwal.id_stasiun_tujuan = stasiun_tujuan.id_stasiun_tujuan and status='Belum Bayar' limit $mulai_dari,$limit " ; 
        ...
        {
          global $kdb1;
        $sql =  "select * from user, pesan, jadwal, stasiun_asal, stasiun_tujuan, kereta where 
        ...
        stasiun_asal.id_stasiun_asal and jadwal.id_stasiun_tujuan = stasiun_tujuan.id_stasiun_tujuan" ; 
